...
static char *first_find_header = NULL;
...
static char *find_first_fit(const size_t size);
...
static void collapse(char *header)
{
    ...
    first_find_header = first_find_header ? ptr_min(first_find_header, header) : header;
    dbg_printf("\tCOLLAPSE SETH FIRST_FIND_HEADER=%p, HEADER=%p\n", first_find_header, header);
    ...
    if (!pre_allocated)
    {
        first_find_header = ptr_min(first_find_header, pre);
        dbg_printf("\tCOLLAPSE SETP FIRST_FIND_HEADER=%p, PRE=%p\n", first_find_header, pre);
    }
}

static char *find_first_fit(const size_t size)
{
    char *cur = first_find_header ? first_find_header : prologue_header;
    while (!is_end(cur))
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if (aligned_size > origin_size - 16)
    {
        ...
        if (next_size)
        {
            if (aligned_size + 16 <= origin_size + next_size)
            {
                ...
                first_find_header = first_find_header ? ptr_min(first_find_header, next_header) : next_header;
    ...
    // |header | payload | footer | header | payload'(x) | footer |
    // x + 8 == origin_size - 8 - size
